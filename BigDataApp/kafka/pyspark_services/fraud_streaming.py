...
def process_batch(batch_df, batch_id):
    ...
    1. Compute 10-minute rolling features per user
    2. Apply ML model
    3. Write ALL transactions to 'all_transactions' topic
    4. Write ONLY FRAUD to 'fraud_alerts' topic
    """
    if batch_df.isEmpty():
        return
    ...
    enriched = batch_with_unix.withColumn(
        ...
        "Failed_Transaction_Count_10m",
        spark_sum(when(col("Risk_Score") >= 0.75, 1).otherwise(0)).over(window_spec)
    ).fillna({
        "Avg_Transaction_Amount_10m": 0.0,
        "Failed_Transaction_Count_10m": 0
    })
    ...
    try:
        ...
        fraud_decisions = predictions.withColumn(
            ...
            "fraud_flag",
            when(col("prob_array")[1] > 0.6, 1).otherwise(0)
        )
        
        fraud_count = fraud_decisions.filter(col("fraud_flag") == 1).count()
        total_count = fraud_decisions.count()
        ...
        all_transactions_output = fraud_decisions.select(
            to_json(struct(
                col("Transaction_ID"),
                col("User_ID"),
                col("Transaction_Amount"),
        ...
        all_transactions_output.write \
            .format("kafka") \
            .option("kafka.bootstrap.servers", "kafka:29092") \
            .option("topic", "all_transactions") \
            .save()
        
        print(f"✅ Sent {total_count} transactions to 'all_transactions' topic")
        
        fraud_only = fraud_decisions.filter(col("fraud_flag") == 1)
        
        if fraud_count > 0:
            fraud_alerts_output = fraud_only.select(
                to_json(struct(
                    col("Transaction_ID"),
                    col("User_ID"),
                    col("Transaction_Amount"),
            ...
            fraud_alerts_output.write \
                .format("kafka") \
                .option("kafka.bootstrap.servers", "kafka:29092") \
                .option("topic", "fraud_alerts") \
                .save()
            
            print(f"🚨 Sent {fraud_count} FRAUD ALERTS to 'fraud_alerts' topic")
        else:
            print("✅ No fraud detected in this batch - nothing sent to 'fraud_alerts' topic")
...
print("="*80)
print("Waiting for transactions from Kafka topic 'transactions'...")
print("📊 ALL transactions → 'all_transactions' topic")
print("🚨 FRAUD ONLY → 'fraud_alerts' topic")
print("Press Ctrl+C to stop\n")
